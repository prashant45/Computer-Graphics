...
mat4.perspective = function (out, fovy, near, far) {
	...
	//
	// ************** Defining the Perspective Projection Matrix (3D case) **************
	//
	var aspect = 1.0;
	var r = aspect*near*Math.tan(fovy/2.0);
	var l = -r;
	var b = near* Math.tan(fovy/2.0);
	var t = -b;
...
Camera3D.prototype.update = function () {
    ...
    this.w = vec3.create();
    this.w[0] = -negViewDir[0]/norm_g;
	this.w[1] = -negViewDir[1]/norm_g;
	this.w[2] = -negViewDir[2]/norm_g;
    vec3.normalize(this.w,this.w);
	
    //
	//************** u vector **************
	//
    var tmp_crossproduct = vec3.create();
    vec3.cross(tmp_crossproduct,this.upVector,this.w);
    var norm_tmp = vec3.length(tmp_crossproduct);
    
	this.u = vec3.create();
	this.u[0] = tmp_crossproduct[0]/norm_tmp;
	this.u[1] = tmp_crossproduct[1]/norm_tmp;
	this.u[2] = tmp_crossproduct[2]/norm_tmp;
